...
from .modeling import ImageEncoderViT3D, MaskDecoder3D, PromptEncoder3D, Sam3D, MaskDecoder3D_MLAM
...
def build_sam3D_vit_b_ori_mlam(checkpoint=None):
    return _build_sam3D_ori_mlam(
        encoder_embed_dim=768,
        encoder_depth=12,
        transformer_dim=384,
        encoder_channels=(384, 384, 384, 384),
        original_image_channels=1,
        encoder_num_heads=12,
        encoder_global_attn_indexes=[2, 5, 8, 11],
        checkpoint=checkpoint,
    )
...
sam_model_registry3D = {
    ...
    "custom_mlam": build_sam3D_vit_b_ori_mlam
}

def _build_sam3D_ori_mlam(
    encoder_embed_dim,
    encoder_depth,
    encoder_num_heads,
    ...
    sam = Sam3D(
        ...
        mask_decoder=MaskDecoder3D_MLAM(
            num_multimask_outputs=0, # No multimask outputs in this case
            transformer_dim=transformer_dim,
            encoder_channels=encoder_channels,
            original_image_channels=original_image_channels,
            iou_head_depth=3,
...
def _build_sam3D_256x256x32(
    ...
    prompt_embed_dim = 384
    image_size = (32, 256, 256)  # 3D image size
    vit_patch_size = 16
    image_embedding_size = image_size // vit_patch_size
    sam = Sam3D(
        image_encoder=ImageEncoderViT3D(
