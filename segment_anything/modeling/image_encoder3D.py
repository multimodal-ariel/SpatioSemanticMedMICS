...
class ImageEncoderViT3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_multiscale=False) -> torch.Tensor:
        ...
        if return_multiscale:
            # build 4 scales: 2×, 4×, 8×, 16×
            d, h, w = x.shape[-3:]
            encoder_feats = (
                F.interpolate(x, size=(d*2, h*2, w*2), mode='trilinear', align_corners=False),
                F.interpolate(x, size=(d*4, h*4, w*4), mode='trilinear', align_corners=False),
                F.interpolate(x, size=(d*8, h*8, w*8), mode='trilinear', align_corners=False),
                F.interpolate(x, size=(d*16, h*16, w*16), mode='trilinear', align_corners=False),
            )
