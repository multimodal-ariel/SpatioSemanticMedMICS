...
class MaskDecoder3D(nn.Module):
    ...
    def forward(
        ...
        if sparse_prompt_embeddings is None:
            masks, iou_pred = self.predict_masks_w_dense(
    ...
    # courtesy: copilot -- to predict using only dense prompt embeddings
    def predict_masks_w_dense(
        self,
        image_embeddings: torch.Tensor,
        image_pe: torch.Tensor,
        dense_prompt_embeddings: torch.Tensor,  # Removed sparse_prompt_embeddings
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Predicts masks using only dense prompts."""
        # Concatenate output tokens (IoU and mask tokens)
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, output_tokens)  # No sparse_prompt_embeddings
        iou_token_out = hs[:, 0, :]
        mask_tokens_out = hs[:, 1 : (1 + self.num_mask_tokens), :]

        # Upscale mask embeddings and predict masks using the mask tokens
        src = src.transpose(1, 2).view(*image_embeddings.shape)
        upscaled_embedding = self.output_upscaling(src)
        hyper_in_list: List[torch.Tensor] = []
        for i in range(self.num_mask_tokens):
            hyper_in_list.append(self.output_hypernetworks_mlps[i](mask_tokens_out[:, i, :]))
